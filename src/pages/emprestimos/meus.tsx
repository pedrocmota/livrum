...
import {getMyLendingProps, IMyLendings} from '../../models/MyLendingProps'

const MeusEmprestimos: React.FunctionComponent<IMyLendings> = (props) => {
  ...
  const refresh = () => {
    axios.get('/api/getMyLendings').then((response) => {
  ...
  return (
    <>
      <Head>
        <title>Livrum - Meus empréstimos</title>
      </Head>
      <Flex
        as="div"
        flexDir="column"
                  ...
                  {lendings.map(((lending) => {
                    return (
                      <tr key={lending.id} onDoubleClick={() => {
                        showLending(props.userData, lending, false, () => {
                          refresh()
                        })
                      }}>
                        ...
                        <td>
                          <Flex flexDir="column">
                            <Button
                              ...
                              paddingLeft="20px"
                              paddingRight="20px"
                              onClick={() => showLending(props.userData, lending, false, () => {
                                refresh()
                              })}
                              _disabled={{
...
export const getServerSideProps: GetServerSideProps = async (context) => {
  ...
  if (session) {
    const LendingProps = await getMyLendingProps(session.userID)
    return {
      props: {
        ...LendingProps
      }
